...
function CreateDF(srcImage: Vec4[][], width: number, height: number) {
  ...
  const INF = 16777216;
  let linears = new Array(height);
  let resultImage = new Array(height);
  for (let i = 0; i < height; i++) {
    linears[i] = new Array(width);
    linears[i].fill(INF);
  ...
  for (let i = 0; i < height; i++) {
    ...
    if (
      ...
      continue;
    linears[i] = MarchParabolas(width, hullVertices[i], hullIntersections[i]);
  }
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++)
